...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CDC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("carpediemcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CDC unit input in lnReqAmount, we generate the URI with CDC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CDC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
